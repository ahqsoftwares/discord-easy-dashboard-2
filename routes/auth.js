...
const Auth = Router()
	...
	clientSecret: req.dashboardConfig.secret,

	code: "query code",
	scope: req.dashboardConfig.scopes.join(" "),
	grantType: "authorization_code",
	
	redirectUri: `${req.dashboardConfig.baseUrl}/auth/login`,
}).then(async (code) => {
            let userData = {
                infos: null,
                guilds: null
            }
            oauth.getUser(code).then(user => userData.infos = user);
            oauth.getUserGuilds(code).then(guilds => userData.guilds = guilds);
